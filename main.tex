...
\todo{Explain: why do we even need alignment?}  \todo{Add proper
  definition of alignment, drop data-type} 
%
...
\end{itemize}
\end{definition}
Note that Definition \ref{def:align} is about \emph{values}, not only
nodes: any two edits \texttt{u ⟼ v} and \texttt{u ⟼ w} are aligned.
%	
...
\emph{source} nodes.
%
However, insert edits may break alignment because they introduce empty
values and can occur at any point in the scripts.
...
\texttt{Nop} edits.
%
Note that \texttt{Nop} edits change neither the \emph{source} nor the
...
No-operations are automatically aligned with inserts, because they
both have the empty value \texttt{⊥} as \emph{source} and in addition
they cannot introduce spurious conflicts because they are
\emph{identity} edits.
