...
%
As a result in the new algorithm using \emph{discrete distance},
a single update edit is preferred to a delete and a subsequent insert.
%
\paragraph{Patch}
...
an ill-typed script.
%
Our library merges objects in this way by means of the function \texttt{gdiff3}:
\begin{minted}{haskell}
gdiff3 :: (Diff a, Diff b) => b → a → b → 
                                 Either [Conflict] (ES '[ a ] '[ b ])
\end{minted}
...
library, with promising results.
We plan to evaluate our algorithm on real world data as future work.
%
