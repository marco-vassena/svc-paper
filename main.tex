...
\begin{abstract}
  ...
  %
  Many VCS systems, including Git and Mercurial, relies on
  ...
  files, hence it might generate spurious conflicts that must be
...
%
VCS provides these features by means of two algorithms, called
...
capabilities.
%
In this paper, we develop a data-type generic version of the
\texttt{diff} and \texttt{diff3} algorithms, that improve their
...
The Comma Separated Values file format (CSV) is a simple text-based encoding of data stored in tables. %b \cite{csv}.
...
%
The source code containing the \texttt{diff} and \texttt{diff3}
...
\end{minted}
Data type \texttt{HList as} is an \emph{heterogeneous} list-like data
structure, because contains trees of different types, namely those
specified by the list of types \texttt{as}.
  %
  The type \texttt{C as a} denotes the constructor of an algebraic
  data type of type \texttt{a} that takes arguments of types
  determined by the list \texttt{as}.
  %
...
\end{minted}
        ...
        witness the possible presence of a node \texttt{C as a}, that given objects 
...
\end{minted}
	...
	Namely, an empty value will consume no input and produce nothing, hence
        ...
        A non-empty value contains a node \texttt{C as a} which consumes 
...
\begin{req}[\texttt{diff} specifications]
  ...
  \texttt{x} and \texttt{y}
  \center{\texttt{e = diff x y ⇔ x ≡ ⟪ e ⟫ ∧ y ≡ ⟦ e ⟧}}
\end{req}
%
...
\end{minted}
	...
	As a result also the semantics of conflict is minimal, but still complete
        ...
        lastly define a function that merges edits.
...
lists of kind \texttt{[ * ]} from the list type of kind \texttt{*}.
%
...
\subsection{Diffing}
Since \texttt{diff3} relies on \texttt{diff} we start by defining a type class
...
%
Method \texttt{(=?=)} checks equality between constructors of the 
...
%
The following function uses \texttt{eqT} from the \texttt{Typeable} superclass
of \texttt{Diff} for that:
\begin{minted}{haskell}
(=??=) :: (Diff a, Diff b) => C xs a → C ys b → Maybe (a :~: b)
_ =??= _ = eqT
\end{minted}
Note that the arguments are ignored and used exclusively to fix which
types must be compared.
Lastly method \texttt{distance} represents a user-customizable distance function
between nodes, used in the \texttt{diff} algorithm's cost model.

\paragraph{Example}
As an example we give the \texttt{Diff} type class instances for list and integers,
which alone allow to diff CSV tables, like those in the initial example.
...
\end{minted} 
%
Note that the instance is polymorphic, so that it can be instantiated
with both \texttt{[Int]} and \texttt{[[Int]]}.
...
that edit operations are not represented by a separate data type, but are
embedded in its constructors.

\begin{minted}{haskell}
data ES xs ys where
  End :: ES [] []
  Ins :: Diff a => C xs a → ES ys (xs :++: zs) → ES ys (a ': zs)
  Del :: Diff a => C xs a → ES (xs :++: ys) zs → ES (a ': ys) zs
  Upd :: Diff a => C xs a → C ys a → ES (xs :++: zs) (ys :++: ws) 
      → ES (a ': zs) (a ': ws)
\end{minted}
The original version cannot be translated directly to Haskell as it
is, due to the limitations of the datatype promotion mechanism
\cite{Yorgey12} and the absence of pi-types.  Symbol \texttt{:++:}
denotes type-level append.
%
...
inserted, or, if the nodes have the same target type, tested by
...
new version, producing two edit scripts that are combined
...
\paragraph{Conflict}
The conflict data type encodes why it is not possible to merge two edits:
\begin{minted}{haskell}
data Conflict where
  InsIns :: (Diff a, Diff b) => C xs a → C ys b → Conflict
  UpdDel :: Diff a => C xs a → C ys a → Conflict
  DelUpd :: Diff a => C xs a → C ys a → Conflict
  UpdUpd :: Diff a => C xs a → C ys a → C zs a → Conflict
\end{minted}

The data type is much simpler compared to the model: it is not indexed
by the base and new values.  This is mainly because the data promotion
mechanism prevents GADTs from being indexed from other GADTs.
...
data ES3 xs where
  ...
  Ins3 :: Diff a => C xs a → ES3 ys → ES3 ys
  Del3 :: Diff a => C xs a → ES3 (xs :++: ys) → ES3 (a ': ys) 
  Upd3 :: Diff a => C xs a → C ys a 
         → ES3 (xs :++: zs) → ES3 (a ': zs) 
\end{minted}
The script is well-typed with respect to the input list simplifying
type checking, since only the output types have to be checked.
%
...
\paragraph{Alignment}
Due to the lack of proper dependent types unaligned scripts are
not automatically ruled out when merging, therefore they have to
be handled explicitly.
%
The following auxiliary functions tests equality between nodes
and throws an error if this is not the case.
\begin{minted}{haskell}
aligned :: (Diff a, Diff b) => C xs a → C ys b → (xs :~: ys , a :~: b)
aligned a b =
  case a =??= b of
    Just Refl →  
      case a =?= b of
...
%
We formalize the semantics of merge and conflict in Agda and
...
library making it possible to put our results into practice, embedded
in application such as VCS.
