...
manipulate at anytime. It is essential that the data is kept consistent, so that changes made
on one device are then replicated on the others. A synchronization algorithm
needs to detect the changes made at each device, combine them and then update each replica with the
merged version. Software industry has first successfully adopted this work-flow by means of Version Control Systems, such as
...
Furthermore even when applied to files encoded as simple text-based formats fail to
correctly merge basic changes. 
The Comma Separated Values file format (CSV) is a simple text-based encoding of data stored in tables \cite{csv}.
In a CSV file every line represents a row of the table and in every row commas are used to separated values from different columns.
For instance a $3 \times 3$ table containing the numbers from 1 to 9 can
...
A user adds a columns filled with zero to the table and saves the new table in the file \texttt{b.csv}:
\begin{figure}[h]
\centering
\begin{minipage}{.25\textwidth}
  \centering
...
\end{minipage}%$
\end{figure} 
Meanwhile another user changes some values in the third column of the original table and saves it as \texttt{c.csv}:
\begin{figure}[h]
\centering
\begin{minipage}{.25\textwidth}
  \centering
...
Unfortunately a line-based algorithms such as \texttt{diff3} fails to automatically merge the changes made to the original table, but raises spurious conflicts.
The algorithm employs the \emph{three-way-merge} technique to compare and merge corresponding lines: a conflict is triggered when the same line is different in all the three versions.

\begin{center}
\begin{tabular}{| c | c | c | c |}
\hline
\texttt{b} & \texttt{a} & \texttt{c} & \texttt{merged} \\ \hline
