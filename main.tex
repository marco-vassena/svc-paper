...
\begin{minted}{haskell}
sameT :: (Diff a, Diff b) => F xs a -> F ys b -> Maybe (a :~: b)
sameT _ _ = eqT
\end{minted}
...
data ES xs ys where
  End :: ES [] []
  Nop :: ES xs ys -> ES xs ys
  Ins :: Diff a => F xs a -> ES ys (xs :++: zs) -> ES ys (a ': zs)
  Del :: Diff a => F xs a -> ES (xs :++: ys) zs -> ES (a ': ys) zs
  Upd :: Diff a => F xs a -> F ys a -> ES (xs :++: zs) (ys :++: ws) 
      -> ES (a ': zs) (a ': ws)
\end{minted}
The original version cannot be translated as it is, because of the 
ambiguity of the type level lists prefixes, that would require
the use of singleton types \cite{Eisenberg12DTPS}.
Symbol \texttt{:++:} denotes type-level append.
...
%
In this paper we have generalized copy to update and we have introduced
a \texttt{Nop} operation , therefore we have to adjust the cost model
accordingly.
...
The edit \texttt{Nop} has weight one, even though it has no
operational effect, because it increases the length of an edit script.
%
In other words for any edit script that contains some \texttt{Nop} an
equivalent script with smaller cost can be obtained by removing each
\texttt{Nop} edit.
%
...
\texttt{sameT}, the first node can be updated with the second.
%
...
using \texttt{⨅}.
%
Note that the \texttt{Nop} edit is never used because it does not
consume neither of the two lists, while at the same time it worsen
the script longer than needed.
%
%
...
ES3 data-type.
\paragraph{Algorithm}
Signature only, considerations about alignment and type checking.
