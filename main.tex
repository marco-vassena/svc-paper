...
%
The rest of the edits progressively consume the remaining nodes, until
...
%
With more precise types, we can define function \texttt{⟪\_⟫}, which
retrieves the \emph{source} object from an edit script:
...
its line-based counterpart, relies on a \texttt{diff} algorithm
...
%
Furthermore, no-operation is an \emph{identity} edit, therefore, as we
show in the next section, it cannot introduce new conflicts, hence it
is an harmless extension.
...
comprises both edits.
%edits: any edit would leave out at least one of them.
%
...
Operator \texttt{⊔₃} merges \emph{aligned} edit scripts\footnote{We
  extend edit scripts with a special constructor \texttt{c ::ᶜ e} for
...
%
If \texttt{ys ≡ zs}, like when \texttt{⊔₃} is firstly called in
\texttt{diff3}, the question is trivial, but the issue repeats when
recursively merging the scripts: there is no ``right'' type that fits
the signature.
%
...
In order to add a merged edit, e.g. \texttt{Ins α}, with \texttt{α : C
  as a}, list \texttt{ws} should have prefix \texttt{as}, i.e.\
\texttt{ws ≡ as ++ ws'}, but \texttt{ws} is existentially quantified:
it is just \emph{some} list.
%
...
\ref{subsec:merge-semantics} makes Theorem \ref{thm:conflict-conds}
straightforward and intuitive, but this does not diminish its
importance.
%
...
gives up \emph{convergence}, i.e.\ the guarantee to always produce a
merged object, and raises a conflict when it is not possible to
soundly merge edits \cite{Pierce07}.
%
...
\end{thm}
Note that edit \texttt{f} represents a proper \emph{change edit}
because \texttt{u ≢ v} ---it could have been an identity edit and
overwritten by another change edit due to axiom \texttt{Id₁}
(\texttt{Id₂}) otherwise.
%
...
Note that, thanks to permissive Requirement \ref{req:diff-specs}, this
property holds also when merging, specifically by instantiating
\texttt{x} with the \emph{base} object, \texttt{e} with the merged
script obtained from \texttt{⊔₃} and \texttt{y} with the merged
object.
...
\end{minted} 
%
Note that when comparing ``constructors'' of primitive types, we have to
...
%
With respect to our work we remark that move edits would weaken the
structural invariant guarantees and would probably complicate
reasoning \cite{Ramsey01}.
	
\paragraph{Formal Model}
The semantics of merging algorithms employed in version control
systems is not formalized, with the notable exception of Darcs
\cite{HomoPatchTheory, CategoryPatchTheory}, but it is usually
understood empirically, leading to severe misconceptions
\cite{PierceDiff3}.
%
...
%both.
%
However the system is so large that it requires automatic techniques
to prove it sound: with only five operations, it counts more than
fifty laws!
%
Our model is instead concise and straightforward to analyze.
%
