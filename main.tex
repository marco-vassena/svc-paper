...
\end{minted}
In Agda, \texttt{Set} is the kind of simple types and \texttt{List
  Set} is the type of a list of simple types.
%
...
of type \texttt{a}, that takes arguments of types given by the list
\texttt{as}.
%
Data type \texttt{HList as} is an \emph{heterogeneous} list,
containing a typed tree for each type in list \texttt{as}.
%
This representation is type-safe because constructor \texttt{Node}
ensures that the constructor and the heterogeneous list share the same
index \texttt{as}, therefore denoting a well-typed application of a
...
and \texttt{\textcolor{blue}{[]} : C [] (List Int)} are the
reification of the cons constructor and the empty list constructor
respectively, specialized without loss of generality to \texttt{List
  Int}.
%
...
We will show in Section \ref{sec:haskell} how to concretely
instantiate \texttt{C} with specific data types, in the following
model we will just leave it abstract as a module parameter, together
with a decidable heterogeneous equality operator.
...
%
  ...
  category.
	...
	while preserving type-safety. \todo{Explain more!}
\begin{minted}{agda}
