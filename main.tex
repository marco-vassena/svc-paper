...
representation.
%
For example, construct \texttt{Just} takes a single argument of type
\texttt{a} and yields a value of type \texttt{Maybe a}, for any type
\texttt{a}, is represented by \texttt{C [a] (Maybe a)}.
%
...
\end{minted}
In Section \ref{sec:haskell} we show how to concretely implement them.

\paragraph{Well-Typed Rose Trees}
Data type \texttt{HTree a} is a tree-like data structure that
represents an algebraic data type of type \texttt{a}, 
using constructor denoted by \texttt{C as a}.
...
\end{minted}
%
This representation is type-safe because constructor \texttt{Node}
requires the same index \texttt{as} in the constructor representation
and in the heterogeneous list, therefore denoting a well-typed
...
%
We firstly describe a simply typed version of edit and edit scripts and
then add indexes to make them well-typed.
...
All the possible combinations of source and target value states, i.e.\
...
called \emph{identity} edits ---observe that they have type \texttt{v
  ⟼ v}.
%
...
%
The objects are not plain trees, but rather \emph{lists} of trees,
which are used as \emph{stacks} of unprocessed nodes, pushed and
popped according to the edit operations found in the script.
%
...
More precisely each tree in the source and target stacks is traversed
and consumed in depth-first preorder by the edits of the script. 
%
...
instead.
%
An update edit pops and pushes from both stacks and a no-operation
edit leave both stacks unchanged.
%
In principle it should be possible to retrieve the source and target
objects from a script, by inverting the process that we have just
described.
%
Unfortunately an un-typed edit script does not guarantee that its
operations will produce well-typed tree and not even that they can
be performed at all.
%
For instance the top of the stack might contain trees of the wrong
type, that do not match those of the constructor at hand, or it might
try to pop trees from an empty stack.
%
Guaranteeing such invariants is the bread and butter of
dependently-typed programming, as we proceed to show in the next
section.

\todo{Type-safe version}

%  while
% preserving type-safety. \todo{Explain more!}
...
\end{minted}
	No-operations and insert edits have no source node,
        ...
        object.
        %
        \todo{Stress on well-typedness of the construction!  HList is
          not just a mere list, but a list of trees!}
        %
...
and has the the following signature: \todo{Discussion about
...
\section{Generic diff$_3$}
	...
	to solve it manually.
        ...
        \todo{Remark that the three-way technique is attractive and
          we simply want to apply it to structured data. Maybe in intro?}
	...
	\subsection{Alignment}
        ...
        \todo{Explain: why do we even need alignment?}
        \todo{Add proper definition of alignment, drop data-type}
...
\end{minted}
        \todo{Proper definition of \texttt{⊴} and proper symbol}
